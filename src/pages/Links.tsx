...
const Links = () => {
  ...
  const onSubmit = async (data: LinkFormData) => {
    ...
    try {
      if (editingLink) {
        await api.put(`/outside-links/${editingLink.id}`, data);
        toast.success("تم تحديث اللينك بنجاح");
      } else {
        await api.post("/outside-links", data);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {links.map((link) => (
            ...
            >
              ...
              <p className="text-gray-300 mb-2">{link.description}</p>
              <a
                href={link.value}
                target="_blank"
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-yellow-500 h-24 resize-none"
                />
                {errors.description && (
                  <p className="text-red-400 text-sm mt-1">
